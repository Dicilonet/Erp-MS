  ...
  return (
    <section id="contact" className="py-16 lg:py-24">
      <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8 text-center">
        <h2 className="text-3xl font-bold tracking-tight text-emerald-700 dark:text-emerald-400">Ponte en Contacto</h2>
        <p className="mt-2 text-lg text-slate-600 dark:text-slate-400">
          ¿Tienes preguntas o quieres proponer una colaboración? Escríbenos.
        </p>
        <form onSubmit={onSubmit} className="mt-10 text-left space-y-4" noValidate>
          ...
          <div>
            ...
            <Textarea id="message" name="message" placeholder="¿Cómo podemos ayudarte?" className="mt-1" />
          </div>
          <Button type="submit" disabled={sending} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white">
            {sending ? 'Enviando...' : 'Enviar Mensaje'}
          </Button>
          {sent && <p role="status" className="text-center text-sm text-green-700 font-medium">¡Gracias! Hemos recibido tu mensaje.</p>}
        </form>
      </div>
    </section>
  );
